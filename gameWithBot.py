...
def getState(map, enemies, players, costMap):
    botplayers = players.copy()
    botplayers = botplayers[-1:]
    mapMatrix, enemiesMatrix, playersMatrix, costMapMatrix = getData(map, enemies, botplayers, costMap)
...
def game(seed, players_amount, enemies_amount, map_size):
    ...
    ##
    lastUpdateTimes = [pygame.time.get_ticks() for _ in range(0, players_amount + 1)]
    ##
    ...
    for i in range(0, players_amount + 1):
...
def step(
    ...
    while len(usersKeys) < len(todos_jugadores)-1:
    ...
    action = testTrain.get_action(state)
    ...
    #Move PLayers
    for index in range(0,len(todos_jugadores)-1):
